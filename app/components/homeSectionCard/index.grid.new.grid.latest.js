...
export default function HomeSectionCard(props) {
  const {
    item,
    index,
    classes = "min-h-[70vh] md:min-h-[calc(100vh-180px)] mt-10",
    innerClasses = "",
  ...
  return (
    <div
      ...
      ref={domRef}
      className={`md:h-fit h-[60vh] cursor-default min-h-[min(70vmin,calc(100vh-180px))] rounded-xl overflow-hidden mt-10 ${classes} shadow-2xl relative border border-solid border-zinc-500/80 hover:border-white hover:shadow-2xl ${
        isSafari ? "" : "grayscale-card"
      } delay-200 ${isVisible ? "grayscale-card-hover" : ""}`}
      style={
        {
    ...
    >
      <div
        className={`p-3 h-full min-h-[min(70vmin,calc(100vh-180px))] mt-10 ${classes} ${innerClasses} rounded-xl flex items-end backdrop-blur-md delay-200 ${
          isVisible && !isSafari ? "!backdrop-blur-0" : ""
        } transition-all duration-1000`}
      >
        <div className="absolute top-0 right-0 h-[66%] left-0 rounded-[0.75rem] flex items-center w-full">
          <img
            src={`/images/${item.image}.png`}
            className={"object-contain md:scale-100 scale-150"}
          />
        </div>
        {/* <div className="absolute right-0 bottom-0 left-0 top-0 bg-light-gradient rounded-[1rem]"></div> */}
        {/* <div className="absolute top-0 right-0 bottom-0 left-0 bg-color-gradient rounded-[0.75rem]"></div> */}
        <div className="absolute right-0 bottom-0 left-0 top-0 bg-dark-gradient rounded-[0.75rem]"></div>
        {item.truncateDescription ? (
          <div className={`absolute right-0 bottom-0 left-0 top-0 transition-all duration-200 delay-200 bg-top-dark-gradient rounded-[0.75rem]`}></div>
        ) : (
          <></>
        )}
        <div
        ...
        >
          ...
          <div
            ...
            className={`mt-2 mb-5 text-zinc-200 drop-shadow-2xl ${
              ...
              item.truncateDescription ? "truncate-3-lines !mb-2 !mt-0" : ""
            } ${index % 2 === 0 ? "" : ""}`}
          >
          ...
          {item.action ? (
            ...
            >
              <Button
                onClick={() => {
                  item.url ? window.open(item.url) : item.action();
                }}
